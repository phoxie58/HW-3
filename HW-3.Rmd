...
* Roughly how much time you spent on this HW: 9
* What gave you the most trouble:The dates for the Jukebox set  
...
kable(summary(model.all))
all <- coefficients(model.all)
kable(1/(1+exp(-all)))
```

To be perfectly honest, I am unsure as to how to interpert each individual coefficient. 
...
```{r, echo=FALSE, fig.width=12, fig.height=6}
profiles <- profiles %>% 
  mutate(p.hat = fitted(model.all), na.rm == TRUE)

model.all.na <- glm(female ~ height + has.wine + is.smoker + athletic + rep.inc + is.admin + is.med + is.ed + is.tech + is.construction, data=profiles, family=binomial, na.pass=TRUE)
summary(model.all.na)
```
This code is not running... it seams to be because the regression dropped the na.s I tried to fix it...
...
ggplotly(bg.time)
```
There seems to be no real correlation between Bitcoin and Gold Prices. 
